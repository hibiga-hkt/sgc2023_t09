...
public class Egg : MonoBehaviour
{
    ...
    void Update()
    {
        if(m_MoveCnt < 0)
        {
            ...
            kids.Set(transform.position, new Vector3(-2.0f - Kids.SpawnCnt / (gamemanager.HeightCnt)
                , -((gamemanager.HeightCnt - 1) * 0.5f) + (Kids.SpawnCnt % (gamemanager.HeightCnt)), 0.0f));

            Kids.SpawnCnt = Kids.SpawnCnt + 1;
